...
<style>
   ...
   <div>   
         <div class="myDownTitle"><img class=myDownImg src="/resources/images/myDown.png"></div>
         <div align="center">
         <br>
         <table>
            <tr style="border: #fff">
               <td style="border: #fff; width: 20%;"><img src="/resources/images/Favorite.png"></td>
               <td style="border: #fff"><label style="color: #0101DF; font-weight: bold ">원하는 컨텐츠 창에서 다운로드 버튼을 누르신 후 내려받으시면 편리합니다.</label>
               <br>
               <label style="font-size: 10px">▶</label> 다운로드 서비스의 정보는 서버에 저장되기 떄문에 다른 컴퓨터에서 보시더라도
                <br>&nbsp;&nbsp; 
      ...
      <table class="custom-table">
         ...
         <c:forEach items="${Paginglist }" var = "lis" >
               <tr>
                  <td>
                     <input type="checkbox" name="fileList" value="${lis.bno }" class="select_remove" style="text-align: center">
                  </td>
  ...
  <div class="paginate">
   ...
   <!-- 선택된 항목없이 삭제 눌렀을경우 방지  -->
    $(".remove").click(function(f){
      ...
      if($("input:checkbox[name=fileList]:checked").length == 0){
          alert("삭제할 항목을 선택해주세요.");
          return;
       }else{
         $("#removeForm").attr("action","/info/removeFiles?pageNum="+${pageMaker.cri.pageNum }+"&amount="+${pageMaker.cri.amount }).submit();
       } 
   });
